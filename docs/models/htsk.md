...
- Use `paper_strict=True` in `HTSKClassifier` or `HTSKRegressor` to enforce
  the paper protocol defaults at estimator level:
  `n_mfs=30`, `mf_init="kmeans"`, `sigma_scale=1.0`, `rule_base="coco"`,
  `epochs=200`, `learning_rate=1e-2`, and `batch_size=512`.
- In `paper_strict=True`, conflicting values for these parameters raise
  `ValueError`.
- The strict trainer uses Adam and applies the paper batch fallback
  `min(N_t, 60)` when `batch_size > N_t`.
- Validation split is not auto-generated by the model/estimator; pass
  `x_val` and `y_val` explicitly when you want early stopping based on
  validation metrics.
...
- For protocol-level reproducibility, prefer `paper_strict=True`; outside that
  mode, highFIS remains configurable and not tied to a single experimental
  setup.
