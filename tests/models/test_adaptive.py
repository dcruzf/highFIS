...
from highfis.memberships import CompositeGaussianMF, GaussianMF
...
def _build_adatsk_input_mfs(n_inputs: int = 3, n_rules: int = 2) -> dict[str, list[CompositeGaussianMF]]:
    return {
        f"x{i + 1}": [CompositeGaussianMF(mean=float(j), sigma=1.0, eps=0.0001) for j in range(n_rules)]
...
def test_adatsk_classifier_can_disable_zero_init() -> None:
    ...
    model = ADATSKClassifierModel(
        _build_adatsk_input_mfs(n_inputs=2, n_rules=2), n_classes=2, zero_consequent_init=False
    )
    weight = cast(torch.Tensor, model.consequent_layer.weight).detach()
...
def test_admtsk_classifier_can_disable_zero_consequent_init() -> None:
    model = ADMTSKClassifierModel(_build_input_mfs(), n_classes=2, zero_consequent_init=False)
    weight = getattr(model.consequent_layer, "weight", None)
    assert isinstance(weight, torch.Tensor)
    assert not torch.allclose(weight, torch.zeros_like(weight))
...
def test_admtsk_regressor_can_disable_zero_consequent_init() -> None:
    model = ADMTSKRegressorModel(_build_input_mfs(), rule_base="coco", zero_consequent_init=False)
    weight = getattr(model.consequent_layer, "weight", None)
    assert isinstance(weight, torch.Tensor)
    assert not torch.allclose(weight, torch.zeros_like(weight))
...
def test_admtsk_classifier_accepts_custom_t_norm_fn() -> None:
    model = ADMTSKClassifierModel(_build_input_mfs(), n_classes=2, t_norm=DombiTNorm(lambda_=1.5))
    x = torch.randn(4, 3)
    out = model.forward(x)
...
def test_admtsk_regressor_accepts_custom_t_norm_fn() -> None:
    model = ADMTSKRegressorModel(_build_input_mfs(), t_norm=DombiTNorm(lambda_=1.5))
    x = torch.randn(4, 3)
    out = model.forward(x)
