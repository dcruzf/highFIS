...
class TestTSKClassifierEstimator:
    def test_fit_predict_score_kmeans(self) -> None:
        ...
        est = TSKClassifier(n_mfs=2, mf_init="kmeans", epochs=5, learning_rate=0.01, random_state=7, batch_size=16)
        est.fit(x, y)
    ...
    def test_fit_predict_score_fcm(self) -> None:
        x, y = _make_clf_dataset(80)
        est = TSKClassifier(n_mfs=2, mf_init="fcm", epochs=5, learning_rate=0.01, random_state=7, batch_size=16)
        est.fit(x, y)
        proba = est.predict_proba(x)
        pred = est.predict(x)
        score = est.score(x, y)
    ...
    def test_fit_predict_grid(self) -> None:
        x, y = _make_clf_dataset(80)
        est = TSKClassifier(n_mfs=2, mf_init="grid", epochs=5, learning_rate=0.01, random_state=7, batch_size=16)
        est.fit(x, y)
        pred = est.predict(x)
        assert pred.shape == (x.shape[0],)
    ...
    def test_validates_input_config_length(self) -> None:
        ...
        est = TSKClassifier(input_configs=[InputConfig(name="x1", n_mfs=2)], batch_size=16)
...
class TestTSKRegressorEstimator:
    def test_fit_predict_score_kmeans(self) -> None:
        ...
        est = TSKRegressor(n_mfs=2, mf_init="kmeans", epochs=5, learning_rate=0.01, random_state=7, batch_size=16)
        est.fit(x, y)
        pred = est.predict(x)
        assert pred.shape == (x.shape[0],)
        r2 = est.score(x, y)
    ...
    def test_fit_predict_score_fcm(self) -> None:
        x, y = _make_reg_dataset(80)
        est = TSKRegressor(n_mfs=2, mf_init="fcm", epochs=5, learning_rate=0.01, random_state=7, batch_size=16)
        est.fit(x, y)
        pred = est.predict(x)
        assert pred.shape == (x.shape[0],)
        r2 = est.score(x, y)
    ...
    def test_fit_predict_grid(self) -> None:
        x, y = _make_reg_dataset(80)
        est = TSKRegressor(n_mfs=2, mf_init="grid", epochs=5, learning_rate=0.01, random_state=7, batch_size=16)
        est.fit(x, y)
        pred = est.predict(x)
        assert pred.shape == (x.shape[0],)
